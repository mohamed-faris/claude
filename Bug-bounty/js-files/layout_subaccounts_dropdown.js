var LayoutSubaccounts = function() {
    ...
    var switchAccount = function(anchor, payload, fallbackUrl) {
        ...
        if (!payload.LOGIN_AS) {
            HTML.handleError({
                'error': 'Unable to switch subaccounts right now. Please refresh and try again.'
            });
            return false;
        }
        ...
        var resp = HTML.ajax('post', END_POINTS.urls()['Login'], payload, null, false, "Loading...");
    ...
    var get_contents = function() {
        ...
        search
            ...
            .on('input.layoutSubaccounts keyup.layoutSubaccounts', function() {
                ...
                listElements.each(function() {
                    var listItem = $(this);
                    var matches = (listItem.attr('data-search-text') || '').indexOf(searchText) !== -1;
    ...
    return {
        ...
        handlelogin: function(e) {
            ...
            payload.STACK = 'STACK';
            return switchAccount(anchor, payload, Globals.BACKEND + END_POINTS.fronts()['Account Summary']);
        },
