...
! function(c, M) {
    ...
    function W() {
        ...
        for (var a, A, o = L(), b = o.length, p = function(c) {
                for (var M, a, A = c.length, o = {}, b = [], p = 0; p < A; p++)
                    for (M in a = e[c[p].offset] || {}) a.hasOwnProperty(M) && (o[M] = !0);
                for (p in o) o.hasOwnProperty(p) && b.push(n[p]);
                return b
            }(o), i = [], z = 0; z < p.length; z++) {
    ...
    function X(c) {
        var M, a, A, o;
        for ("string" == typeof c && (c = [c]), M = 0; M < c.length; M++) o = B(a = (A = c[M].split("|"))[0]), p[o] = c[M], n[o] = a,
            function(c, M) {
                var a, A;
    ...
    function s(c, M) {
        c = B(c);
        var a, A = p[c];
        return A instanceof f ? A : "string" == typeof A ? (A = new f(A), p[c] = A) : i[c] && M !== s && (a = s(i[c], s)) ? ((A = p[c] = new f)._set(a), A.name = n[c], A) : null
    }

    function m(c) {
        var M, a, A, o;
        for ("string" == typeof c && (c = [c]), M = 0; M < c.length; M++) A = B((a = c[M].split("|"))[0]), o = B(a[1]), i[A] = o, n[A] = a[0], i[o] = A, n[o] = a[1]
    }
    ...
    function T(c) {
        X(c.zones), m(c.links),
            function(c) {
                var M, a, A, o;
                if (c && c.length)
                    for (M = 0; M < c.length; M++) a = (o = c[M].split("|"))[0].toUpperCase(), A = o[1].split(" "), z[a] = new N(a, A)
    ...
    function C(c) {
        ...
        parse: function(c) {
            for (var M, a, A, o = +c, b = this.offsets, p = this.untils, i = p.length - 1, z = 0; z < i; z++)
                if (M = b[z], a = b[z + 1], A = b[z ? z - 1 : z], M < a && C.moveAmbiguousForward ? M = a : A < M && C.moveInvalidForward && (M = A), o < p[z] - 6e4 * M) return b[z];
            return b[i]
        },
        abbr: function(c) {
    ...
    }, C.version = "0.5.32", C.dataVersion = "", C._zones = p, C._links = i, C._names = n, C._countries = z, C.add = X, C.link = m, C.load = T, C.zone = s, C.zoneExists = E, C.guess = function(c) {
        return M && !c || (M = W()), M
    }, C.names = function() {
        var c, M = [];
        for (c in n) n.hasOwnProperty(c) && (p[c] || p[i[c]]) && n[c] && M.push(n[c]);
        return M.sort()
