...
jQuery(function($) {
    ...
    $(document).on('click', '.table-remove', function() {
        if ($(this).hasClass('confirm')) {
            ...
            $('<a href="javascript:void(0);" class="table-remove-cancel is-hidden">Cancel</a>').insertAfter(this);
    ...
    $(document).on('click', '.table-remove-cancel', function() {
        $(this).parent().find('.table-remove.confirm').removeClass('confirm').text('Ã— Remove');
    ...
    var handleCheckedState = function(a, g, t) {
        if (t == null)
            t = this;
        $(t).parent().addClass((t.checked) ? 'checked' : '').removeClass((t.checked) ? '' : 'checked');
        var td = $(t).parents('td');
        td.find('.allow-notify-label').text((t.checked) ? 'Enabled' : 'Disabled');
        td.find('.allow-notify-label').addClass((t.checked) ? 'success' : 'disabled').removeClass((t.checked) ? 'disabled' : 'success');
    };
    ...
    $(document).on('click', '.inline-editing .btn', function() {
        $(this).parent().parent().removeClass('inline-editing').addClass('inline-disabled');
    ...
    $(document).on('click', '.card--tab-list .tab, #mobile-tab .tab', function() {
        if (!$(this).hasClass('active')) {
            var tab = $(this).attr('data-tab');
            $(this).parent().find('.tab').removeClass('active');
            $(this).parent('li').parent('.dropdown-menu').find('.tab').removeClass('active');
    ...
    $(document).on('click', '.card--add .users-app-dropdown, .inline-editing .users-app-dropdown', function(e) {
    ...
    $(document).on('click', '.users-app-dropdown', function(e) {
        if ($(this).hasClass('is-open')) {
            ...
            $('.users-app-dropdown--options').remove();
            $('<ul class="users-app-dropdown--options is-trans"><li class="admin"><a href="javascript:void(0);"><h6>Admin</h6><p>Full access except billing/plan management.</p></a></li><li class="reports"><a href="javascript:void(0);"><h6>Reports</h6><p>Reports section only.</p></a></li></ul>').insertAfter(this); //extracted <li class="owner"><a href="javascript:void(0);"><h6>Owner</h6><p>Full control panel access.</p></a></li>
            setTimeout(function() {
    ...
    $(document).on('click', '.users-app-dropdown--options .admin a', function(e) {
        $('.users-app-dropdown--options').parent().find('.users-app-dropdown').html('Admin<input type="hidden" name="user_access" value="admin"/>');
        $('.users-app-dropdown--options').remove();
        $('.users-app-dropdown.is-open').removeClass('is-open');
        e.stopPropagation();
    });

    $(document).on('click', '.users-app-dropdown--options .reports a', function(e) {
        $('.users-app-dropdown--options').parent().find('.users-app-dropdown').html('Reports<input type="hidden" name="user_access" value="reports"/>');
        $('.users-app-dropdown--options').remove();
        $('.users-app-dropdown.is-open').removeClass('is-open');
        e.stopPropagation();
    });
    ...
    $(document).on('click', '.refreshable', function() {
        if ($(this).hasClass('checkable')) {
            $(this).text('Checkingâ€¦').removeClass('error');
            //} else if ($(this).hasClass('verifiable')) {
            //  $(this).text('Verifyingâ€¦').removeClass('error');
        }
    });
    ...
    $(document).on('change', '.rate-limit-defaults select', function() {
        ...
        if ($(this).val() == 'unlimited') {
            ...
            $('.rate-limit-defaults').find('input[name="' + prefix + '_relay_limit"]').val('');
            $('.rate-limit-defaults').find('input[name="' + prefix + '_relay_limit"]').prop('disabled', true);
        } else {
            $('.rate-limit-defaults').removeClass('is-unlimited');
            $('.rate-limit-defaults').find('input[name="' + prefix + '_relay_limit"]').prop('disabled', false);
        }
    });
    ...
    $(document).on('click', '.rate-limit-defaults .change-save', function() {
        ...
        inp[prefix + '_relay_limit'] = $('input[name="' + prefix + '_relay_limit"]').val();
        if (inp[prefix + '_relay_limit'] < 0) {
            $('<span class="inline-error" style="display: none;">The limit cannot be negative.</span>').insertAfter('input[name="' + prefix + '_relay_limit"]');
            fdin();
            fdout(); //clear existing errors.
            return;
        }
        $(this).parent().addClass('is-closed').removeClass('is-open');
        inp[prefix + '_relay_limit_period'] = $('select[name="' + prefix + '_relay_limit_period"]').val();
        var resp = HTML.ajax('PUT', END_POINTS.urls()['Authentication'], inp, null, true);
        resp.success(function(data) {
            if (data.status == 'OK') {
                btn.parent().prepend('<span class="inline-message" style="display: none;">Saved changes</span>');
                Custom.def_rl_period = inp[prefix + '_relay_limit_period'];
                Custom.def_rl_limit = inp[prefix + '_relay_limit'];
                $('.current_limit').text(HTML.format_ratelimit(Custom.def_rl_limit, Custom.def_rl_period));
            } else {
                btn.parent().prepend('<span class="inline-error" style="display: none;">Error Occurred</span>');
                HTML.handleError(data);
            }
            fdin();
            fdout();
    ...
    $(document).on('click', 'input[name="unsubscribe_enabled"]', function() {
        ...
        $this.parents('.unsubscribes').toggleClass('is-closed').toggleClass('is-open');
    ...
    $(document).on('click', '.rate-limit input[type="checkbox"]', function() {
        var ea = function() {
            if ($(this).prop('name') == 'ratelimit_limit' && $('.bootbox-body .rate-limit select').val() == 'unlimited') {} else
                $(this).prop('disabled', $('.form-group.rate-limit').hasClass('is-default'));
            if ($('.form-group.rate-limit').hasClass('is-default')) {
                $(this).val(($(this).prop('name') == 'ratelimit_limit') ? Custom.def_rl_limit : Custom.def_rl_period);
            }
        };

        if ($('.form-group.rate-limit').hasClass('is-default')) {
            $('.form-group.rate-limit').removeClass('is-default').addClass('is-custom');
            $('.form-group.rate-limit').find('.form-control').each(ea);
        } else {
            $('.form-group.rate-limit').addClass('is-default').removeClass('is-custom');
            if (Custom.def_rl_period == 'unlimited')
                $('.form-group.rate-limit').addClass('is-unlimited');
            else
                $('.form-group.rate-limit').removeClass('is-unlimited');
            $('.form-group.rate-limit').find('.form-control').each(ea);
        }
    });
    ...
    $(document).on('click', '.password-show-hide', function() {
        if ($(this).hasClass('is-hidden')) {
            ...
            $(this).parent().find('input').each(function() {
                $('<input class="form-control input-md" type="text" />').attr({
                    name: this.name,
                    value: this.value
                }).insertBefore(this);
            }).remove();
            ...
            $(this).parent().find('input').each(function() {
                $('<input class="form-control input-md" type="password" />').attr({
                    name: this.name,
                    value: this.value
                }).insertBefore(this);
            }).remove();
    ...
    $(document).on('click', '.fullkey-show-hide', function() {
        if ($(this).hasClass('is-hidden')) {
            $(this).removeClass('is-hidden').addClass('is-shown');
            $(this).parent().find('input[id=api_key]').each(function() {
                $(this).val($(this).attr('full-data')).prop('readonly', $(this).data('isnew'));
            });
        } else {
            $(this).removeClass('is-shown').addClass('is-hidden');
            $(this).parent().find('input[id=api_key]').each(function() {
    ...
    var smtp_box = function(data, what) {
        ...
        var default_ippool = data != null ? data.results['default_ippool'] : Custom.default_ippool;
        var method = 'POST';
        ...
        var default_html;
        var default_text;
        var unsubscribe_html = default_html = "<br>\r\n<p><a href=\"" + unsubscribe_tag + "\">unsubscribe</a></p>";
        var unsubscribe_text = default_text = "To unsubscribe click: " + unsubscribe_tag;
        ...
        if (user) {
            ...
            if (user.unsub_tag != null) {
                ...
                unsubscribe_html = default_html = "<br>\r\n<p><a href=\"" + unsubscribe_tag + "\">unsubscribe</a></p>";
                unsubscribe_text = default_text = "To unsubscribe click: " + unsubscribe_tag + "";
            }
        ...
        var set_tab_class = function(e) {
            ...
            $(el).parents('.bootbox-tabs').parent().removeClass().addClass(target + '-open');
        };

        var tablist = $('<ul></ul>').addClass('bootbox-tabs').attr('role', 'tablist');
        if (what == 'salesforce') {
            tablist.append(
                $('<li></li>').addClass('bootbox-tabs__tab active').append(
                    $('<a></a>').attr('href', '#details').attr('role', 'tab').attr('data-toggle', 'tab').text('Details').on('click', set_tab_class)
                ));
        } else {
            tablist.append(
                $('<li></li>').addClass('bootbox-tabs__tab active').append(
                    $('<a></a>').attr('href', '#details').attr('role', 'tab').attr('data-toggle', 'tab').text(what === 'ipaddr' ? 'IP Address Details' : what === 'apikey' ? 'API Details' : 'SMTP Details').on('click', set_tab_class)
                ));
        }
        ...
        if (what == 'apikey') {
            ...
            tablist.append(
                $('<li></li>').addClass('bootbox-tabs__tab').append(
                    $('<a></a>').attr('href', '#permissions').attr('role', 'tab').attr('data-toggle', 'tab').text('Permissions').on('click', set_tab_class)
                )
            );

            // create the content for the permission tab
            permissions_tab = $('<div></div>').attr('role', 'tabpanel').addClass('tab-pane user-tab-permissions').attr('id', 'permissions');
            permissions_tab.append(
                $('<div></div>').addClass('permissions').append(
                    $('<div></div>').addClass('form-group no-margin-bottom').append(api_users_build_permissions(data.results.api_index))
                )
            );
        }
        ...
        if (what != 'salesforce')
            tablist.append(
                $('<li></li>').addClass('bootbox-tabs__tab').append(
                    $('<a></a>').attr('href', '#unsubscribe').attr('role', 'tab').attr('data-toggle', 'tab').text('Unsubscribe Footer').on('click', set_tab_class)
                ));


        tablist.append(
            $('<li></li>').addClass('bootbox-tabs__tab').append(
                $('<a></a>').attr('href', '#tracking').attr('role', 'tab').attr('data-toggle', 'tab').text('Tracking & Status').on('click', set_tab_class)
            ));

        tablist.append(
            $('<li></li>').addClass('bootbox-tabs__tab').append(
                $('<a></a>').attr('href', '#archiving').attr('role', 'tab').attr('data-toggle', 'tab').text('Advanced').on('click', set_tab_class)
            ));

        var wrapper = $('<div></div>');
        if (what == 'salesforce') {
            wrapper.append(
                $('<p></p>').text('This user is directly linked to a Salesforce account. You can optionally specify a rate limit, or choose advanced options.')
            ).append(tablist);
        } else if (what == 'ipaddr') {
            wrapper.append(
                $('<p></p>').text('Edit the IP address details below. Optionally set a rate limit, unsubscribe footer, or choose advanced options.')
            ).append(tablist);
        } else {
            if (what == 'apikey') {
                wrapper.append(
                    $('<p></p>').text('Edit the API key rate limit below. You can also specify endpoint permissions and an unsubscribe footer, ' +
                        'which will be automatically added to the end of every email.')
                ).append(tablist);
            } else {
                wrapper.append(
                    $('<p></p>').text('Edit the userâ€™s credentials and rate limit below. You can also specify an unsubscribe footer, which will be automatically added to the end of every email.')
                ).append(tablist);
            }
        }

        var page = $('<form></form>').addClass('form-horizontal').prop('id', 'smtp-user-edit');
        ...
        var details_tab = $('<div></div>').attr('role', 'tabpanel').addClass('tab-pane user-tab-details active').attr('id', 'details');
        if (user) {
            details_tab.append($('<input></input>').prop('type', 'hidden').prop('name', 'edit').val(user.auth_uid));
        }
        ...
        if (what == 'smtp-auth') {
            ...
            details_tab.append(
                $('<div></div>').addClass('form-group').append(
                    $('<label></label>').addClass('col-md-4 control-label new-label').text('Username')
                )
            );
            $(details_tab.find('.form-group')[0]).append(
                $('<div></div>').addClass('col-md-5').append(
                    $('<input></input>').prop('type', 'text').addClass('form-control input-md').prop('autocomplete', 'off').prop('name', 'auth_username')
                )
            );
            if (user) {
                details_tab.find('input[name="auth_username"]').val(user.username);
            } else {
                if (data && data.results.prefilled_username !== undefined && data.results.prefilled_username !== null) {
                    details_tab.find('input[name="auth_username"]').val(data.results.prefilled_username);
                    $('<p class="info-right">(We\'ve provided a suggested username)</p>').insertAfter(details_tab.find('input[name="auth_username"]'));
                }
            }
            ...
            details_tab.append(
                $('<div></div>').addClass('form-group').append(
                    $('<label></label>').addClass("col-md-4 control-label new-label").text('Password')
                ).append(
                    $('<div></div>').addClass('col-md-5').append(
                        $('<input></input>').prop('type', 'text').addClass('form-control input-md').prop('autocomplete', 'off').prop('name', 'auth_password').val(password)
                    )
                )
            );

            if (user) {
                details_tab.find('input[name="auth_password"]').prop('type', 'password');
            }
            details_tab.find('input[name="auth_password"]').after(
                $('<a></a>').addClass('password-show-hide is-shown').append(
            ...
            if (data) {
                details_tab.find('.password-show-hide').addClass('is-hidden').removeClass('is-shown');
            }
            ...
            details_tab.append(
                $('<div></div>').addClass('form-group').append(
                    $('<label></label>').addClass('col-md-4 control-label new-label').text('IP Address')
                )
            );
            $(details_tab.find('.form-group')[0]).append(
                $('<div></div>').addClass('col-md-5').append(
                    $('<input></input>').prop('type', 'text').addClass('form-control input-md').prop('autocomplete', 'off').prop('name', 'ipaddress')
                )
            );

            if (data) {
                $(details_tab.find('.form-group')[0]).append(
                    $('<input></input>').prop('type', 'hidden').prop('name', 'auth_uid').val(user.auth_uid)
                );
            ...
            var page = $('<div></div>').addClass('form-horizontal').prop('id', 'smtp-user-edit');
            page.append($('<div></div>').addClass('modal-error').css('margin-top', 10));

            // Salesforce newuser
            //extract the email part of the username
            if (user.username.slice(0, 9) == '/SF-init/') user.username = user.username.slice(9);

            var fg = ($('<div class=form-group></div>').append(
                $('<label></label>').addClass('col-md-5 control-label new-label').text('Activation Address:')
            )).append(
                $('<div></div>').addClass('col-md-6 copy-field').append(
                    $('<input onclick="this.focus();this.select()" readonly="readonly" class="form-control activ-address input-md"><button onclick="copyToClipboard(\'.activ-address\')" type="submit">Copy Link</button>').val(user.username)
                )
            );
            ...
            details_tab.append(
                $('<div></div>').addClass('form-group').append(
                    $('<label></label>').addClass('col-md-4 control-label new-label').text('Salesforce ID')
                )
            ).append(
                $('<div></div>').addClass('col-md-5').append(
                    $('<span></span>').text(user.username)
                )
            );
        } else if (what === 'apikey') {
            ...
            details_tab.append(
                $('<div></div>').addClass('form-group').append(
                    $('<label></label>').addClass('col-md-4 control-label new-label').text('API Key')
                )
            );
            $(details_tab.find('.form-group')[0]).append(
                $('<div></div>').addClass('col-md-5').append(
                    $('<input></input>').prop('type', 'text').addClass('form-control input-md').prop('autocomplete', 'off').prop('name', 'username').prop('readonly', true)
                )
            ).append(
                $('<div></div>').addClass('col-md-4 collapse-left').append(
                    $('<input></input>').prop('type', 'hidden').val(_api_key)
                ).append(
                    $('<a></a>').addClass('btn button-download copy-button copy-button-modal mobile-hide').text('Copy')
                )
            );
            ...
            if (data || newApiKey !== null) {
                ...
                $(details_tab.find('.form-group')[0]).append(
                    $('<input></input>').prop('type', 'hidden').prop('name', 'auth_uid').val(username)
                );
                $(details_tab.find('.form-group')[0]).append(
                    $('<input></input>').prop('type', 'hidden').prop('name', 'api_hash').val(api_hash)
                );
        ...
        // description
        var desc = $('<div></div>').addClass('form-group').append(
            $('<label></label>').addClass('col-md-4 control-label new-label').text('Description'));
        var descinput = $('<input></input>').prop('type', 'text').addClass('form-control input-md').prop('autocomplete', 'off').prop('name', 'comment').prop('placeholder', 'Optional');
        ...
        if (Custom.hasippools) {
            var ippools = $('<select></select>').prop('name', 'ippool').addClass('form-control');
            for (var i = 0; i < Custom.ippool.length; i++) {
                var poolHasIps = Custom.ippool[i][2];
                if (poolHasIps) {
                    ippools = ippools.append($('<option></option>').val(Custom.ippool[i][0]).prop('selected', ((Custom.ippool[i][0] == selected_ippool) ? true : false)).text("" + Custom.ippool[i][1]));
                }
            }
            details_tab.append(
                $('<div></div>').addClass('form-group details-ippool').append(
                    $('<label></label>').addClass('col-md-4 control-label new-label').text('IP Pool')
                ).append(
                    $('<div></div>').addClass('col-md-5').append(ippools)
                )
            );
            setTimeout(function() {
        ...
        if (!Custom.available_periods.filter(function(r) {
                return r[0] == ratelimit_period;
            }).length) {
            Custom.available_periods.push([ratelimit_period, 'Per "' + ratelimit_period + '"', 9e9]);
        }
        // populate the rate limit dropdown.
        var opts = $();
        for (var i = 0; i < Custom.available_periods.length; i++) {
            var opt = $('<option></option>').val(Custom.available_periods[i][0]).text(Custom.available_periods[i][1]);
        ...
        details_tab.append(
            $('<div></div>').addClass('form-group rate-limit').append(
                $('<label></label>').addClass('col-md-4 control-label new-label').text('Rate Limit')
            ).append(
                $('<div></div>').addClass('col-md-2').append(
                    $('<input></input>').prop('type', 'number').addClass('form-control').prop('name', 'ratelimit_limit')
                )
            ).append(
                $('<div></div>').addClass('col-md-3 collapse-left').append(
                    $('<select></select>').addClass('form-control').prop('name', 'ratelimit_period').append(opts)
                )
            ).append(
                $('<div></div>').addClass('col-md-4 collapse-left').append(
                    $('<label></label>').addClass('ratelimit-label').append(
                        $('<input></input>').prop('type', 'checkbox').addClass('ratelimit_default').prop('name', 'ratelimit_default')
                    ).append(
                        ' Use default'
                    )
                )
        ...
        if (ratelimit_limit == '' || ratelimit_limit == null || ratelimit_default != '') {
            details_tab.find('input[name="ratelimit_limit"]').prop('disabled', true);
        }

        //Unsubscribes
        var unsub_tab = $('<div></div>').attr('role', 'tabpanel').addClass('tab-pane user-tab-unsubscribe').attr('id', 'unsubscribe');
        var domains = $();
        for (var i = 0; i < Custom.unsub_domains.length; i++) {
            var domain = $('<option></option>').val(Custom.unsub_domains[i][0]).text(Custom.unsub_domains[i][1]);
        ...
        unsub_tab.append(
            $('<div></div>').addClass('unsubscribes').append(
                $('<div></div>').addClass('form-group no-margin-bottom').append(
                    $('<label class="col-md-4 control-label new-label">Add&nbsp;Footer</label>')
                ).append(
                    $('<div></div>').addClass('col-md-7').append(
                        $('<label></label>').addClass('inline-checkbox').append(
                            $('<input></input>').prop('type', 'checkbox').prop('name', 'unsubscribe_enabled')
                        ).append(
                            ''
                        )
                    ).append(
                        $('<small></small>').append(
                            ...
                            $('<a></a>').prop('href', 'https://support.smtp2go.com/hc/en-gb/articles/223087607').prop('target', '_blank').text("(What's this?)")
                        )
                    )
                ).append(
                    $('<div id="unsub_dd"></div>').css('display', 'none').addClass('form-group-inner toggle').append(
                        $('<label></label>').addClass('col-md-4 control-label new-label').html('Domain')
                    ).append(
                        $('<div></div>').addClass('col-md-5').append(
                            $('<select></select>').addClass('form-control smtp-user-domain').prop('name', 'unsubscribe_domain').append(domains)
                        ),
                        $('<div></div>').addClass('col-md-3').append(
                            $('<span class="also">This is optional.</span>')
                        )
                    )
                ).append(
                    $('<div></div>').addClass('form-group-inner toggle').append(
                        $('<div class="col-md-4 control-label new-label"></div>').append(
                            $('<label>HTML</label>')
                        ).append(
                            $('<a></a>').text('use default').on('click', set_default).data('value', default_html).data('target', 'textarea[name="unsubscribe_html"]')
                        )
                    ).append(
                        $('<div></div>').addClass('col-md-9').append(
                            $('<textarea></textarea>').addClass('form-control').prop('name', 'unsubscribe_html')
                        )
                    )
                ).append(
                    $('<div></div>').addClass('form-group-inner toggle').append(
                        $('<div class="col-md-4 control-label new-label"></div>').append(
                            ...
                            $('<a></a>').text('use default').data('value', default_text).data('target', 'textarea[name="unsubscribe_text"]').on('click', set_default)
                        )
                    ).append(
                        $('<div></div>').addClass('col-md-9').append(
                            $('<textarea></textarea>').addClass('form-control').prop('name', 'unsubscribe_text')
                        )
                    )
                ).append(
                    $('').append(
        ...
        // Advanced tab
        var archive_tab = $('<div></div>').attr('role', 'tabpanel').addClass('tab-pane user-tab-archiving').attr('id', 'archiving');
        var content = '<div class="archiving">';
        if (archive_opts_visible) {
            content += '<div class="form-group no-margin-bottom">' +
        ...
        content += '<div class="form-group no-margin-bottom" style="margin-top: 10px;">' +
        ...
        if (show_bounce_notifications === true) {
            ...
            } else { // bounce notifications enabled by default for ip/smtp but no api keys
                if (what === "apikey") {
                    enabled = false;
                    bounces_expanded = false;
                } else {
            ...
            $(document).on('click', '#bounce_notices_email', function() {
                setTimeout(function() {
                    ...
                    var allBounceItems = $(document).find('.bounces-options label input[type="radio"]');
            ...
            setTimeout(function() {
                $('[name="forward_bounces_enabled"]').parents('.tab-content').addClass('has-bounces');
            }, 1000);

            content += '<div class="form-group no-margin-bottom ' + (enabled ? 'is-expanded' : '') + '">' +
                '<label class="col-md-3 control-label new-label" style="padding-top:0; margin-top: -2px;">Bounce Notifications</label>' +
                '<div class="col-md-9">' +
                '<label class="inline-checkbox ' + (enabled ? 'is-open' : 'is-closed') + '"><input type="checkbox" name="forward_bounces_enabled" ' + (enabled ? 'checked' : '') + '/></label>' +
                '<p style="font-size:13px;">Enable this to receive a notification email whenever an email bounces.</p>' +
                '<div class="bounces-options">' +
                '<label class="' + (checkOriginal ? 'is-active' : '') + '"><input type="radio" name="fwdbounces" value="original" ' + (checkOriginal ? 'checked' : '') + '>Send bounce notification emails to the original sender</label><br>' +
                '<label class="' + (checkOther ? 'is-active' : '') + ' input-activate"><input type="radio" name="fwdbounces" value="other" ' + (checkOther ? 'checked' : '') + '>Send bounce notification emails to:</label>' +
                '<input type="text" id="bounce_notices_email" name="fwdbounces_to" placeholder="example@domain.com" value="' + fwdbounces_to + '">' +
                '</div></div></div>';
        }
        ...
        if (archive_enabled) {
            archive_tab.find('.archiving .inline-checkbox.checkbox-archive').addClass('is-open');
            archive_tab.find('input[name="archive_enabled"]').prop('checked', true);
        } else {
            archive_tab.find('.archiving .inline-checkbox.checkbox-archive').addClass('is-closed');
        }
        ...
        // Tracking
        var tracking_tab = $('<div></div>').attr('role', 'tabpanel').addClass('tab-pane user-tab-tracking').attr('id', 'tracking');
        var content = '<div class="tracking">' +
        ...
        // Status bits
        var userStatusAllowed = (user !== null ? (user.mail === null ? true : user.mail != 'BLOCKED') : true);
        var userEnabled = (user !== null ? (user.enabled === null ? true : user.enabled) : true);
        ...
        var contentMessages = {
            ...
            'sandboxed': 'Sending is <span class="emphasis green">allowed</span>, but <span class="emphasis orange">emails won\'t be delivered</span> to the recipient.<ul class="dropdown-info-list"><li>Emails will show on your Activity page with a <span class="extra-strong">Rejected</span> status.</li><li>Emails will still be delivered to any email addresses using the <span class="extra-strong">Email Auditing</span> feature.</li></ul>'
        };

        content += '<div class="form-group no-margin-bottom is-expanded">' +
            '<input type="hidden" name="smtp_enabled" value="' + userStatus + '"/>' +
            ...
            '<span class="selected" style="text-transform:capitalize">' + userStatus + '</span>' +
            '</div>' +
            '<dl class="mode-dropdown-option is-active">' +
            '<div class="dropdown-item blue ' + (userStatus == 'allowed' ? 'is-active' : '') + '">' +
            '<dt>Allowed</dt>' +
            '<dd>Sending is <span class="emphasis green">allowed</span>.' + '<span class="hide">' + contentMessages['allowed'] + '</span></dd>' +
            '</div>' +
            '<div class="dropdown-item red ' + (userStatus == 'blocked' ? 'is-active' : '') + '">' +
            '<dt>Blocked</dt>' +
            '<dd>Sending is <span class="emphasis red">denied</span>.' + '<span class="hide">' + contentMessages['blocked'] + '</span></dd>' +
            '</div>' +
            '<div class="dropdown-item orange ' + (userStatus == 'sandboxed' ? 'is-active' : '') + '">' +
            '<dt>Sandboxed</dt>' +
            '<dd>Sending is <span class="emphasis green">allowed</span>, but <span class="emphasis orange">emails won\'t be delivered</span>, and will show as Rejected. Email Auditing still functions.' + '<span class="hide">' + contentMessages['sandboxed'] + '</span></dd>' +
            '</div>' +
            '</dl>' +
        ...
        if (open_tracking_enabled) {
            tracking_tab.find('input[name="open_tracking_enabled"]').prop('checked', true);
            tracking_tab.find('input[name="open_tracking_enabled"]').parent('.inline-checkbox').removeClass('is-closed').addClass('is-open');
        }
        if (click_tracking_enabled) {
            tracking_tab.find('input[name="click_tracking_enabled"]').prop('checked', true);
            tracking_tab.find('input[name="click_tracking_enabled"]').parent('.inline-checkbox').removeClass('is-closed').addClass('is-open');
        }
        ...
        var tab_content_aug = '';
        if (!archive_opts_visible) { // archiving is disabled, height needs to be less
            tab_content_aug = ' min';
        }

        //submit
        page.append(
            $('<div></div>').addClass('tab-content' + tab_content_aug).append(
                details_tab
            ).append(
                permissions_tab
            ).append(
            ...
            $('<div></div>').addClass('form-group modal-body-footer').append(
                what === 'apikey' ? '<span>Check out our <a href="https://developers.smtp2go.com/" target="_blank">API Documentation</a> if you\'re unsure about any options</label></span>' : '',
                $('<input></input>').prop('type', 'button').addClass('smtp-modal-save btn blue').val('Save').data('method', method).data('what', what)
            )
        );
        wrapper.append(page);
        ...
        if (user && 'password' in user && user.password !== null && user.password.length >= 1)
            $('.password-gen').addClass('hide');
        else
            $('.password-gen').removeClass('hide');
    ...
    $(document).on('click', '.smtp-modal-save', function() {
        ...
        HTML.ajax(method, END_POINTS.urls()[endpoint] + '?ignoreme=' + Custom.getid(), $('#smtp-user-edit').serialize(), function(data) {
            if (data['status'] == 'OK') {
    ...
    $(document).on('click', '.users-smtp-edit', function(e) {
        ...
        if (what == 'salesforce-newuser' || what === undefined) {
            ...
            request.success(function(data) {
                if (data.status == 'OK') {
                    ...
                    if ($this.hasClass('btn')) {
                        $('.users-modal').find('.password-show-hide').removeClass('is-hidden').addClass('is-shown');
                    }



                    if (what == 'salesforce-newuser') {
                        $('.form-horizontal').before('<p>Follow the step-by-step instructions for Salesforce integration with our <a target="_blank" href="https://www.smtp2go.com/setupguide/salesforce/" >Salesforce setup guide</a>. Use the activation email shown below as part of the setup process.</p>');
            ...
            var request = HTML.ajax('GET', END_POINTS.urls()['IP Authentication'] + '?username=' + id + Custom.getid(), null, null, true, 'Loading...');
            request.success(function(data) {
                if (data.status == 'OK') {
                    data.message = null;
                    HTML.set_time(data);
            ...
            var request = HTML.ajax('GET', END_POINTS.urls()['API Keys'] + '?username=' + id + Custom.getid(), null, null, true, 'Loading...');
            request.success(function(data) {
                if (data.status == 'OK') {
                    data.message = null;
                    HTML.set_time(data);
    ...
    $(document).on('click', '#custom-dkim .verifiable', function() {
        bootbox.dialog({
            ...
            message: '<p>To authenticate emails for your domain using SPF and DKIM, Mandrill requires you to verify your domain. By verifying your domain, you confirm ownership of that domain and prevent other Mandrill accounts from sending mail authenticated by your domain. Once you verify a domain, other Mandrill users cannot have their emails signed by that domain unless they also verify it.</p><p>Enter an email address for the domain you wish to verify. Weâ€™ll send a confirmation email with a link to confirm ownership.</p>' +
    ...
    $(document).on('click', '#dedicated-ips .add-ip', function() {
        bootbox.dialog({
            ...
            message: '<p>Each dedicated IP address is charged at $29 per month. If you add an IP address part way through your billing month, you will only be charged for the relevant portion of that month.</p>' +
            ...
            buttons: {
                save: {
                    ...
                    callback: function() {
                        $('<tr class="created"><td data-title="IP Address"><span>New One</span></td><td class="table-action"><a href="#" class="table-remove">&times; Remove</a></td></tr>').prependTo($('.card[data-pool="main"] table tbody'));
                        $('.created').animate({
                            backgroundColor: '#FFFFFF'
                        }, 1000, function() {
                            $('.created').removeClass('created').removeAttr('style');
    ...
    $(document).on('click', '#dedicated-ips .add-pool', function() {
        bootbox.dialog({
            ...
            message: '<p>A description of what a pool is and how it costs nothing goes here.</p>' +
            ...
            buttons: {
                save: {
                    ...
                    callback: function() {
                        ...
                        $('<article class="card" data-pool="test-pool"><div class="table-action pool-action"><a href="#" class="table-remove pool-remove">Ã— Remove</a></div><h2 class="card--title is-underlined">' + poolname + '</h2><table><thead><tr><th colspan="2">IP Address</th></tr></thead><tbody></tbody></table></article>').appendTo($('#dedicated-ips'));
                    }
                }
            }
        });
    ...
    if ($(window).width() < 768) { // Smaller than 768
    } else { // Larger than 768
        var isDown = false;
        var isOver = false;
        $(document).on('mousedown', '#dedicated-ips tbody tr td[data-title="IP Address"] span', function(e) {
            $('body').addClass('dragging');
            $(this).addClass('floating');
            $('table').addClass('floating-droppable-area');
            $(this).parent().parent().parent().parent().removeClass('floating-droppable-area');
            $('.floating-droppable-area').before('<div class="drop-ip-address-here">Drop IP address here</div>');
            isDown = true;
        });
    ...
    $(document).on('click', '#email-testing .preview', function() {
        bootbox.dialog({
            ...
            message: '<p>Check how your email looks in your own inbox.<br>Where shall we send your preview?</p>' +
    ...
    $(document).on('click', '#email-testing .filter', function() {
        bootbox.dialog({
            ...
            message: '<p>Find out if your recipients will actually receive your emails. Improve your email delivery with this spam test, and see exactly how popular spam filters will respond to your email.</p>' +
    ...
    $(document).on('click', '#email-testing .screenshot', function() {
        bootbox.dialog({
            ...
            message: '<p>Different email apps display emails in different ways. This screenshot test will show you exactly how different email apps will display your email, to make sure things look right for everyone.</p>' +
