var Daterange = {
    init: function() {
        ...
        $('.daterange-group').each(function() {
            ...
            // Multiple components on the same page each call Daterange.init() once when they
            // mount (e.g. Sent and Received tabs both have their own date-range picker, kept
            // alive together via v-show rather than v-if). Without this guard, every later
            // init() call re-processes elements already set up by an earlier one â€” rebinding
            // duplicate event handlers and causing every subsequent change to fire multiple
            // times over.
            if ($group.data('daterangeGroup')) {
                return;
            }
            ...
            var $moduleCustomFromTimeGroup = $moduleCustomFrom.find('.daterange-group__time');
            var $moduleCustomFromTimeLabel = $moduleCustomFromTimeGroup.find('.daterange-group__time-label');
            var $moduleCustomFromTimeValue = $moduleCustomFromTimeLabel.find('.daterange-group__time-value');
            var $moduleCustomFromTimeValueHours = $moduleCustomFromTimeValue.find('.daterange-group__time-value-hours');
            var $moduleCustomFromTimeValueMinutes = $moduleCustomFromTimeValue.find('.daterange-group__time-value-minutes');
            var $moduleCustomFromTimeValuePeriod = $moduleCustomFromTimeValue.find('.daterange-group__time-value-period');
            var $moduleCustomFromTimeInputsWrapper = $moduleCustomFromTimeGroup.find('.daterange-group__time-inputs');
            var $moduleCustomFromTimeInputHours = $moduleCustomFromTimeInputsWrapper.find('.daterange-group__time-input_hours');
            var $moduleCustomFromTimeInputMinutes = $moduleCustomFromTimeInputsWrapper.find('.daterange-group__time-input_minutes');
            var $moduleCustomFromTimeInputPeriod = $moduleCustomFromTimeInputsWrapper.find('.daterange-group__time-input_period');
            var $moduleCustomTo = $moduleCustom.find('.daterange-group__to');
            var $moduleCustomToInput = $moduleCustomTo.find('> input');
            var $moduleCustomToTimeGroup = $moduleCustomTo.find('.daterange-group__time');
            var $moduleCustomToTimeLabel = $moduleCustomToTimeGroup.find('.daterange-group__time-label');
            var $moduleCustomToTimeValue = $moduleCustomToTimeLabel.find('.daterange-group__time-value');
            var $moduleCustomToTimeValueHours = $moduleCustomToTimeValue.find('.daterange-group__time-value-hours');
            var $moduleCustomToTimeValueMinutes = $moduleCustomToTimeValue.find('.daterange-group__time-value-minutes');
            var $moduleCustomToTimeValuePeriod = $moduleCustomToTimeValue.find('.daterange-group__time-value-period');
            var $moduleCustomToTimeInputsWrapper = $moduleCustomToTimeGroup.find('.daterange-group__time-inputs');
            var $moduleCustomToTimeInputHours = $moduleCustomToTimeInputsWrapper.find('.daterange-group__time-input_hours');
            var $moduleCustomToTimeInputMinutes = $moduleCustomToTimeInputsWrapper.find('.daterange-group__time-input_minutes');
            var $moduleCustomToTimeInputPeriod = $moduleCustomToTimeInputsWrapper.find('.daterange-group__time-input_period');
            var $moduleCustomTimeButton = $moduleCustom.find('.daterange-group__time-button');
            ...
            var $moduleCustomInputs = $().add($moduleCustomFromInput).add($moduleCustomToInput);
            ...
            function renderTime($groupBlock) {
                var groupType = $groupBlock.hasClass('daterange-group__from') ? 'FROM' : 'TO';
                var date = groupType === 'FROM' ? getStartDate() : getEndDate();
                var $valueHours = groupType === 'FROM' ? $moduleCustomFromTimeValueHours : $moduleCustomToTimeValueHours;
                var $valueMinutes = groupType === 'FROM' ? $moduleCustomFromTimeValueMinutes : $moduleCustomToTimeValueMinutes;
                var $valuePeriod = groupType === 'FROM' ? $moduleCustomFromTimeValuePeriod : $moduleCustomToTimeValuePeriod;
                var $hoursInput = groupType === 'FROM' ? $moduleCustomFromTimeInputHours : $moduleCustomToTimeInputHours;
                var $minutesInput = groupType === 'FROM' ? $moduleCustomFromTimeInputMinutes : $moduleCustomToTimeInputMinutes;
                var $periodInput = groupType === 'FROM' ? $moduleCustomFromTimeInputPeriod : $moduleCustomToTimeInputPeriod;
            ...
            function updateNativeInputsRestrictions(daterangepicker, $nativeDateInput, $nativeTimeInput) {
                if (daterangepicker.minDate) {
                    $nativeDateInput.attr('min', daterangepicker.minDate.format('YYYY-MM-DD'));
                    ...
                    if (isTimeSelectionEnabled) {
                        if (
                            moment(
                                $nativeDateInput.val(),
                                'YYYY-MM-DD',
                            ).isSame(daterangepicker.minDate, 'day')
                        ) {
                            $nativeTimeInput.attr('min', daterangepicker.minDate.format('HH:mm'));
                        } else {
                ...
                if (daterangepicker.maxDate) {
                    $nativeDateInput.attr('max', daterangepicker.maxDate.format('YYYY-MM-DD'));
                    ...
                    if (isTimeSelectionEnabled) {
                        if (
                            moment(
                                $nativeDateInput.val(),
                                'YYYY-MM-DD',
                            ).isSame(daterangepicker.maxDate, 'day')
                        ) {
                            $nativeTimeInput.attr('max', daterangepicker.maxDate.format('HH:mm'));
                        } else {
            ...
            function useDaterangepickerDate() {
                var daterangepickerFrom = $moduleCustomFromInput.data('daterangepicker');
                ...
                updateNativeInputsRestrictions(daterangepickerFrom, $moduleNativeFromDateInput, $moduleNativeFromTimeInput);
                updateNativeInputsRestrictions(daterangepickerTo, $moduleNativeToDateInput, $moduleNativeToTimeInput);
            }
            ...
            function initNative() {
                ...
                $moduleNativeFromInputs
                    .on('blur', function() {
                        if (isTimeSelectionEnabled) {
                            setDaterangepickerDate(
                                $moduleCustomFromInput,
                                moment(
                                    $moduleNativeFromDateInput.val() + ' ' + $moduleNativeFromTimeInput.val(),
                                    'YYYY-MM-DD HH:mm',
                                ),
                            );
                        } else {
                ...
                $moduleNativeToInputs
                    .on('blur', function() {
                        if (isTimeSelectionEnabled) {
                            setDaterangepickerDate(
                                $moduleCustomToInput,
                                moment(
                                    $moduleNativeToDateInput.val() + ' ' + $moduleNativeToTimeInput.val(),
                                    'YYYY-MM-DD HH:mm',
                                ).endOf('minute'),
                            );
                        } else {
            ...
            function initCustom() {
                $moduleCustomInputs.each(function() {
                    ...
                    $input.daterangepicker({
                        ...
                        template: '<div class="daterangepicker daterange-group__picker">' +
                ...
                var fromDaterangepicker = $moduleCustomFromInput.data('daterangepicker');
                var toDaterangepicker = $moduleCustomToInput.data('daterangepicker');

                $([fromDaterangepicker, toDaterangepicker]).each(function(i, daterangepicker) {
                    ...
                    (function() {
                        ...
                        daterangepicker.element.on('show.daterangepicker', function() {
                            $document.off('touchend.daterangepicker', daterangepicker._outsideClickProxy);
                ...
                function markInRangeDates(date) {
                    ...
                    if (startDate.isBefore(endDate)) {
                        ...
                        if (date.isAfter(startDate) && date.isBefore(toDaterangepicker.endDate)) {
                            return 'in-range';
                        }
                    }
                ...
                $moduleCustomFromInput.on('hide.daterangepicker', function() {
                    if (fromDaterangepicker.startDate > toDaterangepicker.startDate) {
                        setDaterangepickerDate($moduleCustomToInput, fromDaterangepicker.startDate.clone().endOf('day').startOf('second'));
                    }
                });

                $moduleCustomToInput.on('hide.daterangepicker', function() {
                    if (fromDaterangepicker.startDate > toDaterangepicker.startDate) {
                        setDaterangepickerDate($moduleCustomFromInput, toDaterangepicker.startDate.clone().startOf('day'));
                    }
                });
                ...
                $.each([{
                    ...
                    var $numberInputs = $().add(data.$inputHours).add(data.$inputMinutes);
                    var currentArrowsControlStartTimeoutId;
                    var currentArrowsControlIntervalId;

                    $numberInputs.on('keydown', function(e) {
                        if (e.keyCode >= 48 && e.keyCode <= 57) { // digits 0-9
                            if (e.target.value.length >= 2 && (e.target.selectionEnd - e.target.selectionStart === 0)) {
                                e.target.value = '';
                            }
                        }
                        ...
                        if ((e.key === 'ArrowUp' || e.key === 'ArrowDown')) {
                            ...
                            if (currentArrowsControlStartTimeoutId == null) {
                                ...
                                var nextStep = function() {
                                    var newValue = (parseInt(e.target.value) + 1 * direction).toString();
                                    $el.val(newValue);
                                    $el.trigger('input');
                    ...
                    $numberInputs.on('blur', function(e) {
                        if (e.target.value.length < 2) {
                            e.target.value = Array(2 - e.target.value.length).fill('0').join('') + e.target.value;
                        }
                    });
            ...
            function getEndDate() {
                var date = $moduleCustomToInput.data('daterangepicker').startDate.clone();
                if (!isTimeSelectionEnabled) {
                    date.endOf('day');
                }
                return date;
            ...
            function setDateRestriction(restriction, date) {
                ...
                if (date.isSame(dateStartOfHour) || date.isSame(dateHalfOfHour) || date.isSame(dateEndOfDay)) {
            ...
            function hideTimeIfUnchanged() {
                ...
                if (
                    (startDate.isSame(startDateStartOfDay) || (minDate && startDate.isSame(minDate))) &&
                    (endDate.isSame(endDateEndOfDay) || (maxDate && endDate.isSame(maxDate)))
                ) {
