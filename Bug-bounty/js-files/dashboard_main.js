...
var Custom = function() {
    ...
    function load_dashboard_main(data, success, failure) {
        ...
        data.unix_secs_start = Math.floor(moment().utc().subtract(30, 'days').startOf('day').valueOf() / 1000);
        data.unix_secs_end = Math.floor(moment().utc().endOf('day').valueOf() / 1000);

        var request = HTML.ajax('GET', Globals.BACKEND + END_POINTS.urls()['Account Summary'], data);
    ...
    function setupOnboardingBanner(serverData) {
        ...
        if (serverData.is_subaccount) {
            completedPercentage = (Object.keys(serverData.setup_account).length - 1) * 33;
        ...
        if (completedPercentage < 99) {
            ...
            $.each(serverData.setup_account, function(key, value) {
                ...
                $('#setup-status .onboarding-tab-content.' + key).addClass("is-disabled");
            });
    ...
    function toggleOnboardingPopup() {
        ...
        var request = HTML.ajax('PUT', Globals.BACKEND + END_POINTS.urls()['Account Summary'], reqData);
    ...
    function DesktopVersion() {
        ...
        if (localStorage.getItem("graphcolors.1") === null) {
            var graph_colors_array = ['#A2D4FC', '#1892E6', '#e38b81', '#F0D78E', '#D0BA7B', '#3BB67F', '#B57AD4'];
        ...
        graph_colors_dash = [graph_colors[0], graph_colors[3], graph_colors[2], graph_colors[5], graph_colors[6]];
        ...
        var get_contents = function() {
            ...
            load_dashboard_main(data, function(data) {
                if (data.status == 'OK') {
                    sent_first_email = data.results.sent_first_email;

                    $('.tooltips').tooltip();
                    $('#remaining').html(parseInt(data.results.used).toLocaleString() + '&#47;' + (parseInt(data.results.remaining) + parseInt(data.results.used)).toLocaleString());
                    $('#remaining').removeClass('hidden');

                    var prgress = (parseInt(data.results.used) / (parseInt(data.results.remaining) + parseInt(data.results.used)));

                    if (prgress > 1) {
                        var allocated = (parseInt(data.results.remaining) + parseInt(data.results.used)) * 3;
                        var percentageValue = parseInt(data.results.used) / allocated;
                        $('.progress').addClass('progress-overflow').append('<style>.progress.progress-overflow::after{width: ' + HTML.format_percent(percentageValue, null, 'en-US') + ';}.progress-overflow p.text-center.progress-bar-count{width: ' + HTML.format_percent(percentageValue, null, 'en-US') + ' !important;}</style>');
                        if (percentageValue > 0.79) {
                            $('.progress').addClass('progress-overflow-high');
                        }
                    } else {
                        $('.progress-bar-count').css('width', HTML.format_percent(prgress, null, 'en-US'));
                    }
                    setTimeout(function() {
                        var progWidth = $('.progress p.text-center.progress-bar-count').width();
                        $('.progress-bar-tooltip-container').width(progWidth);
                        if (progWidth < 16) {
                            $('.progress-bar-tooltip-container').append('<style>.progress-bar-tooltip::after {left: calc(' + progWidth + 'px + 6px);}.progress-bar-tooltip{left: -10px;}</style>');
                        } else if (progWidth > 15 && progWidth < 161) {
                            $('.progress-bar-tooltip-container').append('<style>.progress-bar-tooltip::after {left: calc(' + progWidth + 'px / 2 - 3px);}</style>');
                        }
                    }, 300);

                    $('.progress-bar').css('width', HTML.format_percent(prgress, null, 'en-US'));

                    if (END_POINTS.fronts()['Change Plan'] != null && !data.results.hide_changeplan) {
                        $('#update_plan').removeClass('hidden');
                    }

                    var ext = '<sup>th</sup>';
                    if (data.results.next_billing_day == 1 || data.results.next_billing_day == 21 || data.results.next_billing_day == 31)
                        ext = '<sup>st</sup>';
                    else if (data.results.next_billing_day == 2 || data.results.next_billing_day == 22)
                        ext = '<sup>nd</sup>';
                    else if (data.results.next_billing_day == 3 || data.results.next_billing_day == 23)
                        ext = '<sup>rd</sup>';
                    $('#emails-this-billing-month .meta').html('Emails sent this billing month. Resets on ' + data.results.next_billing_monthname + ' ' + data.results.next_billing_day + ext + '.');

                    if (data.results.setup_account["sender_domain_verified"] === undefined || !data.results.setup_account.sender_domain_verified) {
                    ...
                    if (data.results.plan_refresh == '1 day, 0:00:00')
                        $('#emails-this-billing-month .meta').html('Emails sent today. Resets at midnight GMT.');
                    ...
                    bnc.html('<span class="dash-stat-value">' + HTML.format_percent(parseFloat(data.results.bounces.totals.bounce_percent) / 100) + '</span>');
                    if (data.results.bounces.totals.bounce_percent < c_bn_good) {
                        bnc.html(bnc.html() + '<div class="dash-stat-status dash-stat-status-good"><strong>Good</strong><div class="dash-stat-popout"><span>This is the percentage of emails that you have sent in the last 30 days that have bounced back. A reputation of â€˜Goodâ€™ is given for a bounce percentage of up to ' + c_bn_good + '%.<br /><a href="https://support.smtp2go.com/hc/en-gb/articles/223087727-Bounce-Spam-Percentages-and-Ratings" target="_blank">Click here for more info.</a></span></div></div>');
                    } else if (data.results.bounces.totals.bounce_percent < c_bn_bad) {
                        bnc.html(bnc.html() + '<div class="dash-stat-status dash-stat-status-fair"><strong>Fair</strong><div class="dash-stat-popout"><span>This is the percentage of emails that you have sent in the last 30 days that have bounced back. A reputation of â€˜Fairâ€™ is given for a bounce percentage of ' + c_bn_good + '% to ' + c_bn_bad + '%.<br /><a href="https://support.smtp2go.com/hc/en-gb/articles/223087727-Bounce-Spam-Percentages-and-Ratings" target="_blank">Click here for more info.</a></span></div></div>');
                    } else {
                        bnc.html(bnc.html() + '<div class="dash-stat-status dash-stat-status-poor"><strong>Poor</strong><div class="dash-stat-popout"><span>This is the percentage of emails that you have sent in the last 30 days that have bounced back. A reputation of â€˜Poorâ€™ is given for a bounce percentage above ' + c_bn_bad + '%.<br /><a href="https://support.smtp2go.com/hc/en-gb/articles/223087727-Bounce-Spam-Percentages-and-Ratings" target="_blank">Click here for more info.</a></span></div></div>');
                    }

                    var spm = $('#thirty-day-spam-rate .dash-stat-figure');
                    spm.html('<span class="dash-stat-value">' + HTML.format_percent(data.results.spam.totals.spam_percent / 100) + '</span>');
                    if (data.results.spam.totals.spam_percent < c_spam_good) {
                        spm.html(spm.html() + '<div class="dash-stat-status dash-stat-status-good"><strong>Good</strong><div class="dash-stat-popout"><span>This is the percentage of emails that you have sent in the last 30 days that have been marked as spam. A reputation of â€˜Goodâ€™ is given for a spam percentage up to ' + c_spam_good.toFixed(2) + '%.<br /><a href="https://support.smtp2go.com/hc/en-gb/articles/223087727-Bounce-Spam-Percentages-and-Ratings" target="_blank">Click here for more info.</a></span></div></div>');
                    } else if (data.results.spam.totals.spam_percent < c_spam_bad) {
                        spm.html(spm.html() + '<div class="dash-stat-status dash-stat-status-fair"><strong>Fair</strong><div class="dash-stat-popout"><span>This is the percentage of emails that you have sent in the last 30 days that have been marked as spam. A reputation of â€˜Fairâ€™ is given for a spam percentage of ' + c_spam_good.toFixed(2) + '% to ' + c_spam_bad.toFixed(2) + '%.<br /><a href="https://support.smtp2go.com/hc/en-gb/articles/223087727-Bounce-Spam-Percentages-and-Ratings" target="_blank">Click here for more info.</a></span></div></div>');
                    } else {
                        spm.html(spm.html() + '<div class="dash-stat-status dash-stat-status-poor"><strong>Poor</strong><div class="dash-stat-popout"><span>This is the percentage of emails that you have sent in the last 30 days that have been marked as spam. A reputation of â€˜Poorâ€™ is given for a spam percentage above ' + c_spam_bad.toFixed(2) + '%.<br /><a href="https://support.smtp2go.com/hc/en-gb/articles/223087727-Bounce-Spam-Percentages-and-Ratings" target="_blank">Click here for more info.</a></span></div></div>');
                    }
        ...
        var reposition_empty_text = function() {
            if (graph_has_values || sent_first_email || graph == null || graph_is_dummy) {
                return;
            }
        ...
        var init_chart = function(data, first_run) {
            ...
            if (first_run) {
                ...
                if (data.results.emails != null) {
                    for (var i = 0; i < data.results.emails.length; i++) {
                        if (!isNaN(parseInt(data.results.emails[i].emails)) && parseInt(data.results.emails[i].emails) > 0)
                            graph_has_values = true;
                        emails.data.push([data.results.emails[i].date, data.results.emails[i].emails]);
                    }
                }

                if (data.results.bounces.rows != null) {
                    for (var i = 0; i < data.results.bounces.rows.length; i++) {
                        bounces.data.push([data.results.bounces.rows[i].date, data.results.bounces.rows[i].hardbounces + data.results.bounces.rows[i].softbounces]);
                    }
                }

                if (data.results.spam.rows != null) {
                    for (var i = 0; i < data.results.spam.rows.length; i++) {
                        spam.data.push([data.results.spam.rows[i].date, data.results.spam.rows[i].spams]);
                    }
                }
                ...
                if (g_has_open_trackers) {
                    if (data.results.opens.rows != null) {
                        for (var i = 0; i < data.results.opens.rows.length; i++) {
                            opens.data.push([data.results.opens.rows[i].date, data.results.opens.rows[i].opens]);
                        }
                    }
                }
                ...
                if (g_has_click_trackers) {
                    if (data.results.clicks.rows != null) {
                        for (var i = 0; i < data.results.clicks.rows.length; i++) {
                            clicks.data.push([data.results.clicks.rows[i].date, data.results.clicks.rows[i].clicks]);
                        }
                    }
                }

                if (emails.data.length > 0)
                    lines[emails.label] = emails;
                if (bounces.data.length > 0)
                    lines[bounces.label] = bounces;
                if (spam.data.length > 0)
                    lines[spam.label] = spam;
                if (opens.data.length > 0)
            ...
            if (first_run) {
                ...
                $.each(graph.getData().reverse(), function(i, series) {
                    html = '<div class="chart-legend-item"><div id="clicker" class="clicker" style="border:1px solid #ccc; padding:1px; width:14px; height:14px;" onclick="Custom.checkbox(event);" >';
                    html += '<div style="width:4px; height:0; border:5px solid ' + series.color + '; background: ' + series.color + '; overflow:hidden" id="id' + series.label.split(" ").join("_") + '" class="custombox" name="' + series.label + '">';
                    html += '</div></div>';
                    html += '<label class="control-label" href="javascript:void(0);" onclick="Custom.checkbox(event);" for="id' + series.label.split(" ").join("_") + '">' + series.label + '</label></div>';
                    ...
                    if (item) {
                        if (previousPoint != item.dataIndex) {
                            ...
                            if (item.series.label == 'Emails') {
                                text = HTML.format_number(Math.round(y)) + " " + item.series.label + " Sent on " + d;
                            } else {
                                text = HTML.format_number(Math.round(y)) + " " + item.series.label + " on " + d;
                            }
        ...
        function showTooltip(x, y, contents) {
            $('<div id="tooltip" class="tooltip-inner">' + contents + '</div>').css({
                position: 'absolute',
                display: 'none',
                top: y + 5,
                left: x - 80,
    ...
    function MobileVersion() {
        ...
        var graph_colors_dash = [graph_colors[0], graph_colors[3], graph_colors[2], graph_colors[5], graph_colors[6]];
        ...
        function getChartData(serverData) {
            ...
            if (serverData.emails != null) {
                for (var i = 0; i < serverData.emails.length; i++) {
                    emails.data.push([serverData.emails[i].date, serverData.emails[i].emails]);
                }
            }

            if (serverData.bounces.rows != null) {
                for (var i = 0; i < serverData.bounces.rows.length; i++) {
                    bounces.data.push([serverData.bounces.rows[i].date, serverData.bounces.rows[i].hardbounces + serverData.bounces.rows[i].softbounces]);
                }
            }

            if (serverData.spam.rows != null) {
                for (var i = 0; i < serverData.spam.rows.length; i++) {
                    spam.data.push([serverData.spam.rows[i].date, serverData.spam.rows[i].spams]);
                }
            }
            ...
            if (g_has_open_trackers) {
                if (serverData.opens.rows != null) {
                    for (var i = 0; i < serverData.opens.rows.length; i++) {
                        opens.data.push([serverData.opens.rows[i].date, serverData.opens.rows[i].opens]);
                    }
                }
            }
            ...
            if (g_has_click_trackers) {
                if (serverData.clicks.rows != null) {
                    for (var i = 0; i < serverData.clicks.rows.length; i++) {
                        clicks.data.push([serverData.clicks.rows[i].date, serverData.clicks.rows[i].clicks]);
                    }
                }
            }
        ...
        function initBasic() {
            ...
            function getCloseHandler($popup, bindingEvent, closeFn) {
                return function(e) {
                    ...
                    var isCloseButton = $target.parents('.popup-close').length || $target.is('.popup-close');
            ...
            if (g_has_open_trackers) {
                $chartCheckboxes.find('input[data-line="Opens"]').parents('.item').removeClass('hide');
            }

            if (g_has_click_trackers) {
                $chartCheckboxes.find('input[data-line="Clicks"]').parents('.item').removeClass('hide');
            }
        }
        ...
        function renderData() {
            ...
            $usageBar.find('.content').html(used.toLocaleString() + ' &#47; ' + total.toLocaleString());
            ...
            $dashboard.find('.js-resets-on').text(serverData.next_billing_monthname.slice(0, 3) + ' ' + serverData.next_billing_day);
            ...
            var $bounceRatePopup = $dashboard.find('.js-rate-popup[data-name="bounce"]');
            var $bounceRatePopupScore = $bounceRatePopup.find('.score');
            var $bounceRatePopupValues = $bounceRatePopup.find('.values');
            $bounceRateBadge.text(HTML.format_percent(parseFloat(serverData.bounces.totals.bounce_percent) / 100));
            if (serverData.bounces.totals.bounce_percent < rateBounceGood) {
                ...
                $bounceRatePopupValues.text('of ' + rateBounceGood + '% to ' + rateBounceBad + '%');
            } else {
            ...
            $spamRateBadge.text(HTML.format_percent(parseFloat(serverData.spam.totals.spam_percent) / 100));
            if (serverData.spam.totals.spam_percent < rateSpamGood) {
                ...
                $spamRatePopupValues.text('of ' + rateSpamGood + '% to ' + rateSpamBad + '%');
            } else {
            ...
            if (serverData.setup_account["sender_domain_verified"] === undefined || !serverData.setup_account.sender_domain_verified) {
                $('.enable-sender-domain-message').removeClass('hide');
            }
    ...
    function getData() {
        ...
        load_dashboard_main(reqData, function(res) {
            if (res.status == 'OK') {
                serverResponse = res;
    ...
    return {
        init: function() {
            ...
            if (Globals.BACKEND + END_POINTS.fronts()['Subaccounts'] !== "undefined") {
        ...
        handlelogin: function(e) {
            ...
            var resp = HTML.ajax('post', END_POINTS.urls()['Login'], data, null, true, "Loading...");
            resp.success(function(data) {
                HTML.set_time(data);
                window.location = Globals.BACKEND + END_POINTS.fronts()['Account Summary'];
            });
        },
