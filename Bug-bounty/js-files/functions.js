    ...
    return {
        general: function() {
            if (END_POINTS.fronts()['Timezone'] != null)
                $('#current_time').prop('href', Globals.BACKEND + END_POINTS.fronts()['Timezone']);
            ...
            logout_button.off().on('click', function(e) {
                ...
                function clear() {
                    ...
                    if (END_POINTS != null)
                        ...
                        window.location = '/login/?message=1';
                }
        ...
        isProgrammaticClickEvent: function(e) {
            return !e.hasOwnProperty('originalEvent') && !e.hasOwnProperty('clientX') && !e.hasOwnProperty('clientY');
        },
        ...
        RequestError: function(httpObj, textStatus) {
            ...
            if (httpObj.status == 401) {
                var error = '1';
                window.location = END_POINTS.fronts()['Login'] + '?error=' + encodeURI(error);
                return;
            }
            HTML.handleError({
        ...
        handleError: function(data, usepopup, selector) {
            ...
            if (usepopup == null) {
                ...
                if (isGenericError && !data.web_errors) {
                    ...
                    alrtcontainer.append(document.createTextNode(data.error)).append($('<br/>'));
                }
                ...
                if (data.web_errors) //web_errors is a list of objects. some with input name values, some without.
                {
                    for (var i = 0; i < data.web_errors.length; i += 1) {
                        ...
                        if (error.hasOwnProperty('input')) {
                            ...
                            if (inp.parents('form').length && inp.parents('form').data('alert-ignore') != null) {
            ...
            $(selector).removeClass('successful').addClass('error').addClass('visible');
            $(selector).find('.button-close').on('click', function(event) {
        ...
        createInlineLoadingGIF: function(parentSelector, isCentered, customCSS) {
            ...
            if (isCentered) {
                $(parentSelector).append('<div class="loading-spinner" style="' + customCSS + '"><div class="loader"></div></div>');
            } else {
                $(parentSelector).prepend('<div class="loader loader-abs-left loader-modal-footer"></div>');
            }
        },
        ...
        format_percent: function(n, hide, locale) {
            ...
            } else if (HTML.getInternetExplorerVersion() > 0 && n.indexOf('%') === -1) {
                n = n + '%';
            }
            n = n.replace(/\s+/g, '');
            return n;
        ...
        format_date: function(n, output_format, ignore_ms) {
            timezone = null;
            if (output_format != null || /\t$/.test(n)) //if trailing tab exists, assume null.
            { // assume y-m-d format.
                var z = n.split('-');
                var y = z[0];
                var m = z[1];
                ...
                var date = new Date(parseInt(y), parseInt(m) - 1, parseInt(d), parseInt(h), parseInt(M), parseInt(s), parseInt(S));
                if (output_format != null) {
                    if (date.toString == null)
                        console.log("ERROR: NO toString FUNCTION EXISTS. is date.js Included?");
        ...
        format_ratelimit: function(limit, period) { //Format the limit & period into pretty text.
            var res = 'Unlimited',
                periods = [
        ...
        set_time: function(data, selector, noScroll) {
            ...
            if (data != null) {
                if (END_POINTS.fronts !== undefined && END_POINTS.fronts()['Timezone'] != null)
                    $('#current_time').prop('href', Globals.BACKEND + END_POINTS.fronts()['Timezone']);
                if (data['has_back'] != null) {
                    $('#has_back').removeClass('hide');
                    $('#has_back > a').on('click', function(e) {
                        e.preventDefault();
                        var bck = HTML.ajax('post', '/logout/', {
                            'BACK': null
                        }, null, true, "Loading...");
                ...
                if (data.verification != null) {
                    if (data.verification.email_not_verified) {
                        ...
                        $('#email-verification-notice .email-address').text(data.verification.email_address);

                        // handle verification email message links
                        $('.resend-verification-email').unbind().click(function(event) {
                            var request = HTML.ajax('POST', '/onboarding/?action=resend-verify-email', null, null, true, 'Sending...');
                            if (request) {
                        ...
                        $('#setup-status.onboarding-backdrop').addClass('with-email-verification');
                    }
                } else {
        ...
        sub_conform_to: function(data, ztx) {
            ...
            if (ctx == $ && typeof form !== 'undefined') {
                ctx($(form).find('input')).parents('.form-control').removeClass('has-error').removeClass('has-warning');
            }
            ...
            ctx('.automatic[data-display]').each(function(i, e) {
                ...
                if (j.hasClass('money')) // format as money...
                    v = HTML.format_currency(v)
                else if (j.hasClass('cash')) // format as money without leading 'US'...
                    v = (HTML.format_currency(v)).replace(/US/, '')
                else if (j.hasClass('count')) // format as integer...
                    v = HTML.format_number(v)
                else if (j.hasClass('bytecount')) // format as B/K/M/G size...
                    v = HTML.format_byte(v)
                else // no formatting
                    v = v
                ...
                if (j.is('select'))
                    ...
                    if (e.parentNode.parentNode.className == "checker" &&
                        e.parentNode.parentNode.nodeName == 'DIV')
                        e.parentNode.className = e.checked ? 'checked' : undefined;
                } else if (j.is('input') && !j.is('input[type=textarea]'))
                    j.val(v);
                else if (j.hasClass('rawhtml')) // no formatting HTML
                    j.html(v);
                else if (j.is('img'))
                    j.attr('src', v);
                else
            ...
            ctx('.automatic-repeat[data-display]').each(function(i, e) {
                ...
                while (e.nextSibling && e.nextSibling.getAttribute && e.nextSibling.getAttribute('data-copy-of') == e) {
        ...
        init: function() {
            ...
            //
            $('form.auto-submit button[type="submit"]').each(function(index, object) {
        ...
        conform_forms: function(data) {
            ...
            for (var k in data.results) {
                ...
                $('form.autoajax input[name="' + k + '"]').each(
                    function(e, h) {
                        i = $(h)
                        if (i.is(":checkbox")) {
                            if (i.val() == v || (i.val() == 'true' && v)) // fixme - should allow v is an array
                            {
                ...
                $('form.autoajax textarea[name="' + k + '"]').each(
                    function(e, h) {
                        i = $(h)
                        i.val(v)
                        if (typeof(i.data('ui-update')) == 'function') {
                ...
                $('form.autoajax select[name="' + k + '"]').each(
                    function(e, h) {
                        i = $(h)
                        i.val(v)
                        if (typeof(i.data('ui-update')) == 'function') {
        ...
        ajax: function(type, url, data, success, block, blockmessage, async) {
            ...
            if (0 < ['get', 'POST', 'PUT', 'DELETE'].indexOf((type.toUpperCase()))) {
                if (typeof data == 'string') data += '&CSRF_key=' + encodeURIComponent(HTML.csrfkey);
        ...
        search: function(url, method, datatable, post_data, success) {
            ...
            var loopme = function(url, method, datatable, post_data, success) {
                ...
                if (page > 0)
                    post_data.prev_page = ((page - 1) < 0) ? 0 : (page - 1);
                if (post_data.length)
                    post_data.length = 35;
                if (post_data.direction == 0 && page > 0)
                ...
                request.success(function(data) {
                    ...
                    if (((new Date().getTime() / 1000) - start) > 30) //cancel the search if it's been going longer than 30 seconds.
                        HTML.SEARCH_CANCEL = true;

                    if (!finished && !HTML.SEARCH_CANCEL) {
                        post_data['start_exim'] = data.results.startid;
                        post_data['end_exim'] = data.results.data[data.results.data.length - 1]['xid'];
        ...
        makeForm: function(data, submit, url, method) { // make a form with hidden fields and optional submit button.
        ...
        applyDateRangeSelection: function(theTable, dateRange, dateChangeOptFunc, changeOpts) {
        ...
        alertUserInline: function(parentSelector, alertType, messageHTML) {
            ...
            setTimeout(function() {
                $('.footer-note').remove();
                var messageContainer = $('<span></span>').addClass('footer-note label left ' + labelColor).html(messageHTML).css('text-align', 'left');
        ...
        alertUser: function(alertType, message, isModal, selector) {
            ...
            alert.removeClass();
            alert.empty();
            alert.html(HTML.escapeHTML(message) || (alertType == this.alertType.error ? 'Oops something went wrong, please try again or contact support' : 'Updated successfully'));
            ...
            alert.removeClass().addClass(baseAlertClass).addClass(alertType).addClass('visible is-active');
            if (alertType == this.alertType.success || alertType == 'success') setTimeout(function() {
                alert.removeClass('visible is-active')
            }, 7000);
        },
        ...
        processPrimarySearchFieldInput: function(event) {
            ...
            while ((match = queryParser.exec(query)) !== null) {
                ...
                if (match[1] == 'subject') {
                    ...
                    var b = false;
                    if (match[3].toLowerCase() === 'true' || match[3].toLowerCase() === '1') {
                        b = true;
                    }
...
function cache_bust_graph_colors(graph_colors) {
    ...
    if (graph_colors[5] === undefined || graph_colors[5] != '#3BB67F') { // 9ed468 was the initial test color for opens
