...
var HelperMixin = function() {
    ...
    return {
        ...
        clearNotice: function() {
            $('#user-error-alert').removeClass('is-active success error warning info visible');
        },
        ...
        getUnixSecsRangeByDays: function(nDays, client_timezone) {
            ...
            if (client_timezone !== undefined) {
                return {
                    start: moment.tz(tz).subtract(nDays, 'days').startOf('day').utc().unix(),
        ...
        getUnixSecsRangeBetweenDates: function(from, to, client_timezone) {
            ...
            var fromDate = moment.utc(from, 'MM/DD/YYYY').startOf('day').unix() + tzOffset;
...
var AJAXBackendMixin = function() {
    ...
    var _makeRequest = function(method, endpoint, action, payload, successCB, failedCB, quietRequest) {
        ...
        $.ajax(
            ...
            END_POINTS.urls()[endpoint] + '?action=' + encodeURIComponent(action) + '&CSRF_key=' + encodeURIComponent(HTML.csrfkey), {
                ...
                complete: function(xhr, status) {
                    ...
                    if (_tag.log !== undefined) {
                        _tag.log(method + '(' + endpoint + ')[' + action + '] complete <', status, xhr, '>');
                    }
    ...
    return {
        ...
        GET: function(endpoint, action, payload, successCB, failedCB, quietRequest) {
            return _makeRequest('GET', endpoint, action, payload, successCB, failedCB, quietRequest);
        },

        // POST perform a post request to the url specified, passing an optional action/payload
        POST: function(endpoint, action, payload, successCB, failedCB, quietRequest) {
            return _makeRequest('POST', endpoint, action, payload, successCB, failedCB, quietRequest);
        },

        // PUT perform a put request to the url specified, passing an optional action/payload
        PUT: function(endpoint, action, payload, successCB, failedCB, quietRequest) {
            return _makeRequest('PUT', endpoint, action, payload, successCB, failedCB, quietRequest);
        },

        // DELETE perform a delete request to the url specified, passing an optional action/payload
        DELETE: function(endpoint, action, payload, successCB, failedCB, quietRequest) {
            return _makeRequest('DELETE', endpoint, action, payload, successCB, failedCB, quietRequest);
        },
